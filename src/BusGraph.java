...
public class BusGraph {
	...
	public void findTripByTime(int time) 
	{
		...
		else 
		{
			for (int i = 0; i < matchingTrips.size(); i++) 
			{
				...
				// go down the trip until you reach the edge which had the correct arrival time,
				// printing details as you go
	...
	private void readInTrips(BusStop[] stops, String file) 
	{
		...
		BusFileReader c = new BusFileReader(file);
		...
		for (int i = 2; i < list.size(); i++)
		{
			...
			if (from[0].equals(to[0])) 
			{
				...
				{
					stops[fromIndex].addEdge(new BusEdge(tripID, stops[fromIndex], stops[toIndex], departureTime, arrivalTime, seq, toDist - fromDist));
				}
	...
	/**
	 * Return array of bus stops that include part of the input in their name using
	 * a tst
	 * 
	...
	private BusStop[] readInStopData(String file) 
	{
		...
		for (int i = 0; i < stops.length; i++) 
		{
			...
			if (token.length() == 1 && (token.equals("N") || token.equals("E") || token.equals("S") || token.equals("W"))) 
			{
				busStopName = busStopName.substring(2) + " " + token;
			}

			stops[i] = new BusStop(Integer.parseInt(data[0]), code, busStopName, data[3], Double.parseDouble(data[4]), Double.parseDouble(data[5]), data[6], Integer.parseInt(data[8]), parent);
		}
